...
public class GeneralTransloaderTest extends BaseTestCase {
    ...
    public void testReturnsCloneReturnedFromGivenCloningStrategy() throws Exception {
        ...
        CloningStrategy cloningStrategy = new CloningStrategy() {
            public Object cloneObjectUsing(ClassLoader cloneClassLoader, Object original) throws Exception {
                assertSame(expectedOriginal, original);
                assertSame(expectedClassloader, cloneClassLoader);
                return expectedClone;
            }
        };
        assertSame(expectedClone, new ObjectWrapper(expectedOriginal, cloningStrategy, ClassWrapper.getClassLoaderFrom(expectedOriginal)).cloneWith(expectedClassloader));
    }
    ...
    public void testWrapsExceptionThrownByGivenCloningStrategy() throws Exception {
        ...
        Thrower thrower = new Thrower() {
            public void executeUntilThrow() throws Throwable {
                new ObjectWrapper(expectedOriginal, throwingCloningStrategy, ClassWrapper.getClassLoaderFrom(expectedOriginal)).cloneWith(dummyClassLoader);
            }
        };
        assertThrows(new TransloaderException("Unable to clone '" + expectedOriginal + "'.", expectedException), thrower
        );
    }
